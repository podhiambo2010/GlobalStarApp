...
if module == "Dashboard & Arrears":
  ...
  if not unmapped.empty:
    ...
    with col_ua1:
      target_shop = st.number_input(
          "Target Shop #", min_value=1, max_value=22, value=20, key="target_shop_input"
      )
  ...
  st.subheader("🤖 AI Lease Agreement Extractor & OCR")
  uploaded_pdf = st.file_uploader("Upload Signed Lease PDF", type=["pdf"])
  if uploaded_pdf is not None:
    extracted_info = LeaseDocumentAI.parse_lease_pdf(uploaded_pdf)
    st.json(extracted_info)
    if st.button("Register / Update Tenant Master Data"):
      st.success("Tenant successfully profiled and synchronized with master records!")
...
"""
  ...
  selected_shop = st.selectbox(
      "Select Tenant Shop for Individual Action", df_arr["Shop #"].tolist()
  )
